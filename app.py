...
with st.sidebar:
    ...
    with st.container(border=True):
        ...
        st.latex(r'''\alpha_H''')
        alpha_h = st.slider("The probability of fit for type H merchant", alpha_l+0.01, 1.0)
        st.latex(r'''N''')
...
with st.container(border=True):
    ...
    st.latex(r'''\alpha_L \in [\alpha_2, \alpha_H)\ where\ \alpha_2 = \frac{1}{2}[\sqrt{N^2 - 2N(1-\beta)\alpha_H + \alpha^2_H} - (N - \alpha_H)]''')
    alpha_2 = (1/2)*((((n ** 2) - (2 * n * (1 - beta) * alpha_h) + (alpha_h ** 2)) ** 0.5) - (n - alpha_h))
    st.latex("\\alpha_L = "+str(alpha_l)+" \\in ["+str(alpha_2)+", "+str(alpha_h)+")")
    ...
    st.latex(r'''\alpha_2 = \frac{1}{2}[\sqrt{N^2 - 2N(1-\beta)\alpha_H + \alpha^2_H} - (N - \alpha_H)]''')
    st.latex("\\alpha_L = "+str(alpha_l)+" \\in [0, "+str(alpha_2)+")")
    st.latex(r'''and''')
    st.latex(r''' \theta \in (max\{\theta_1, \theta_2\}, 1)''')
    theta_1 = ((p * alpha_h) - (alpha_l * (n + alpha_h)))/((n + alpha_h) * (alpha_h - alpha_l))
    theta_2 = (n * alpha_l * (1 - ((1/2) * beta)))/((alpha_l + ((1/2) * beta * n) *  (alpha_h - alpha_l)))
    st.latex(r'''\theta_1 = ''' + str(theta_1))
    st.latex(r'''\theta_2 = ''' + str(theta_2))
    st.latex("\\theta = "+str(theta)+" \\in (max\\{"+str(theta_1)+", "+str(theta_2)+"\\}, 1)")
    ...
    st.latex("where\\ \\alpha_3 = \\frac{\\sqrt{(N^2 + (N-p)\\alpha_H)^2 + 2p\\beta N\\alpha_H(N+\\alpha_H)}+p\\alpha_H-N(N+\\alpha_H)}{2(N+\\alpha_H)}")
    ...
    st.latex("\\alpha_L = "+str(alpha_l)+" \\in ("+str(alpha_3)+", "+str(alpha_2)+")")    
    ...
    equilibrium = 1 if distortionless_separating else (2 if pooling else (3 if distortion else 0))
    st.text("Equilibrium: "+get_equilibrium(equilibrium))
